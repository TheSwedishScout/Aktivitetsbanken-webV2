...
$rait = curl_init($url); 
	...
	//curl_setopt($rait, CURLOPT_URL, $url);
	//curl_setopt($rait, CURLOPT_CUSTOMREQUEST, 'POST');
	//curl_setopt($rait, CURLOPT_POSTFIELDS, $rait);
	curl_setopt($rait, CURLOPT_RETURNTRANSFER, true); // förväntar sig ett svar 
	curl_setopt($rait, CURLOPT_POSTFIELDS, $postData);
